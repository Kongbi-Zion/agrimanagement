...
// You can make the WhatsApp number configurable via props if needed
interface Props {
  phoneNumber?: string
  message?: string
}
...
const whatsappUrl = computed(() => {
  const baseUrl = `https://wa.me/${props.phoneNumber.replace(/[^\d]/g, '')}`
  return props.message ? `${baseUrl}?text=${encodeURIComponent(props.message)}` : baseUrl
})
</script>
